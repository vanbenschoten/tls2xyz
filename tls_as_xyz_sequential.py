...
def run(pdb_file_name,
  ...
  #Screw rotation
  L_ = r.b_o.L_L.as_sym_mat3()
  Lxx, Lyy, Lzz = L_[0], L_[1], L_[2]
  params.append(Lxx)
  params.append(Lyy)
  params.append(Lzz)
  #Group translation
  V_V_ = r.h_o.V_V.as_sym_mat3()
  Tx, Ty, Tz = V_V_[0], V_V_[1], V_V_[2]
  params.append(Tx)
  params.append(Ty)
  params.append(Tz)
...
def step_i__get_dxdydz(L_L, R_PL, step, sigma, sampling, log, eps=1.e-7):
  """
  Generation of shifts from screw rotations.
  """
  #Update this to read in step number,
  #Define sampling as across 5 sigma
  #Perhaps don't need to calculate probability, as we can calculate it independently for steps that are a function of sigma 
  print_step("step_i__get_dxdydz:", log)
  L_ = L_L.as_sym_mat3()
  Lxx, Lyy, Lzz = L_[0], L_[1], L_[2]
  dx0, dy0, dz0 = 0, 0, 0
  if(abs(Lxx)>eps): dx0 = 2*(step*sigma*Lxx/(sampling-1)) - sigma*Lxx #random.normalvariate(0,Lxx)
  if(abs(Lyy)>eps): dy0 = 2*(step*sigma*Lyy/(sampling-1)) - sigma*Lyy #random.normalvariate(0,Lyy)
  if(abs(Lzz)>eps): dz0 = 2*(step*sigma*Lzz/(sampling-1)) - sigma*Lzz #random.normalvariate(0,Lzz)
...
def step_j(h_o, step, sampling, sigma, log):
  """
  Generate shifts from group translation.
  """
  #Update shifts to also occur in step sizes of 5 sigma/sampling
  print_step("step_j:", log)
  V_V_ = h_o.V_V.as_sym_mat3()
  tx0, ty0, tz0 = 0, 0, 0
  if(V_V_[0] != 0): tx0 = 2*(step*sigma*V_V_[0]/(sampling-1)) - sigma*V_V_[0]
  if(V_V_[1] != 0): ty0 = 2*(step*sigma*V_V_[1]/(sampling-1)) - sigma*V_V_[1]
  if(V_V_[2] != 0): tz0 = 2*(step*sigma*V_V_[2]/(sampling-1)) - sigma*V_V_[2]
